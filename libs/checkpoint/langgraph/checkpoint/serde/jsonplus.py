...
class _RestrictedUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module: str, name: str) -> Any:
        if self.allowed_modules is True:
            pass
        elif self.allowed_modules is None:
            raise pickle.UnpicklingError(
                f"Global and local object imports are forbidden. Blocked {module}.{name}"
            )
        else:
            module_parts = tuple(module.split("."))
            full_parts = module_parts + (name,)
            is_allowed = False
            for allowed_prefix in self.allowed_modules:
                if full_parts[: len(allowed_prefix)] == allowed_prefix:
                    is_allowed = True
                    break
    ...
    def __init__(
        ...
        *,
        ...
        if self.pickle_fallback and self._allowed_pickle_modules is True:
            warnings.warn(
                "JsonPlusSerializer was initialized with pickle_fallback=True and "
                "allowed_pickle_modules=True (unrestricted). This allows arbitrary "
                "code execution if the underlying checkpoint storage is compromised. "
                "Please configure allowed_pickle_modules with a strict allowlist.",
                UserWarning,
                stacklevel=2,
            )
