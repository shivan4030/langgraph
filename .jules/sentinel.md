...
**Vulnerability:** Sensitive headers (such as `x-api-key` and `authorization`) were leaked during cross-origin redirects because manual `Location` header following for SSE reconnects bypassed `httpx`'s built-in header stripping.
